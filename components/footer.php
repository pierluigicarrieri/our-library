<footer>
    <div class="footer-top-wrapper d-flex flex-column align-items-center py-3 bg-dark-subtle">
        <div class="footer-top d-flex justify-content-between w-75 px-5 py-3">
            <div class="px-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quia veritatis ut sapiente exercitationem rem! Ullam, dignissimos. Numquam facilis adipisci perferendis tenetur debitis asperiores ipsa optio, quia modi eaque rerum dolor!</div>
            <div class="px-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi necessitatibus corrupti tempora cum natus sapiente quas repudiandae minus fugiat ipsum dicta dolorum, cupiditate nulla, fugit quod, dolore eaque aut eius?</div>
            <div class="px-5">Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi necessitatibus corrupti tempora cum natus sapiente quas repudiandae minus fugiat ipsum dicta dolorum, cupiditate nulla, fugit quod, dolore eaque aut eius?</div>
        </div>
        <div class="text-center py-3">
            <a href="http://localhost:8888/Personal_Projects/our_library/pages/login.php" class="btn btn-danger">Login</a>
            <a href="http://localhost:8888/Personal_Projects/our_library/pages/logout.php" class="btn btn-danger">Logout</a>
            <a href="http://localhost:8888/Personal_Projects/our_library/pages/insert_book.php" class="btn btn-danger">Add New Book</a>
            <a href="http://localhost:8888/Personal_Projects/our_library/pages/book_insert_result.php" class="btn btn-danger">New Book Added</a>
            <a href="http://localhost:8888/Personal_Projects/our_library/pages/user_library.php" class="btn btn-danger">User Library</a>
        </div>
    </div>
    <div class="footer-bottom text-center p-2 text-light bg-secondary">Copyright &#169; 2024 | Pier & Bubu</div>
</footer>